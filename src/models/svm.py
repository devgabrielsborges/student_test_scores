"""
SVM Training Script for Student Test Scores with Optuna

This script trains an SVR model with hyperparameter tuning
...
def train_with_mlflow(X_train, y_train, X_test, y_test, n_trials=100):
    ...
    experiment = mlflow.set_experiment(
        "Student Scores - SVM Optuna", artifact_location=get_artifact_location()
    )

    with mlflow.start_run(run_name="SVM_Optuna") as run:
        ...
        mlflow.sklearn.log_model(
            best_model,
            "model",
            registered_model_name="SVM_StudentScores_Optuna",
            input_example=X_train.iloc[:5],
            signature=mlflow.models.signature.infer_signature(X_train, y_train),
        )

        # Generate submission file
        generate_submission(best_model, "svm_submission.csv")
        mlflow.log_artifact("svm_submission.csv")
...
def main(n_trials=100):
    ...
    print("=" * 60)
    print("STUDENT TEST SCORES PREDICTION - SVM (OPTUNA)")
    print("=" * 60)
