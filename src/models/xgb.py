"""
XGBoost Training Script for Student Test Scores with Optuna

This script trains an XGBoost Regressor model with hyperparameter tuning
...
def optimize_with_optuna(X_train, y_train, n_trials=100):
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="xgboost_student_scores",
...
def train_with_mlflow(X_train, y_train, X_test, y_test, n_trials=100):
    ...
    experiment_name = "Student Scores - XGBoost Optuna"
    try:
    ...
    with mlflow.start_run(run_name="XGBoost_Optuna") as run:
        ...
        mlflow.xgboost.log_model(
            best_model,
            "model",
            registered_model_name="XGBoost_StudentScores_Optuna",
            input_example=X_train.iloc[:5],
            signature=mlflow.models.signature.infer_signature(X_train, y_train),
        )

        # Generate submission file
        generate_submission(best_model, "xgboost_submission.csv")
...
def main(n_trials=100):
    ...
    print("=" * 60)
    print("STUDENT TEST SCORES PREDICTION - XGBOOST (OPTUNA)")
    print("=" * 60)
