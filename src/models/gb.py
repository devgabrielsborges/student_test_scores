"""
Gradient Boosting Training Script for Student Test Scores with Optuna

This script trains a Gradient Boosting Regressor model with hyperparameter tuning
...
def optimize_with_optuna(X_train, y_train, n_trials=100):
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="gradient_boosting_student_scores",
...
def train_with_mlflow(X_train, y_train, X_test, y_test, n_trials=100):
    ...
    mlflow.set_experiment("Student Scores - Gradient Boosting Optuna")
    ...
    with mlflow.start_run(run_name="GradientBoosting_Optuna") as run:
        ...
        mlflow.sklearn.log_model(
            best_model,
            "model",
            registered_model_name="GradientBoosting_StudentScores_Optuna",
            input_example=X_train.iloc[:5],
            signature=mlflow.models.signature.infer_signature(X_train, y_train),
        )

        # Generate submission file
        generate_submission(best_model, "gradient_boosting_submission.csv")
        mlflow.log_artifact("gradient_boosting_submission.csv")
...
def main(n_trials=100):
    ...
    print("=" * 60)
    print("STUDENT TEST SCORES PREDICTION - GRADIENT BOOSTING (OPTUNA)")
    print("=" * 60)
