"""
Ridge Regression Training Script for Student Test Scores with Optuna

This script trains a Ridge Regression model with hyperparameter tuning
...
def optimize_with_optuna(X_train, y_train, n_trials=100):
    ...
    study = optuna.create_study(
        direction="maximize",
        study_name="ridge_regression_student_scores",
...
def train_with_mlflow(X_train, y_train, X_test, y_test, n_trials=100):
    ...
    mlflow.set_experiment("Student Scores - Ridge Regression Optuna")

    with mlflow.start_run(run_name="RidgeRegression_Optuna") as run:
        ...
        mlflow.sklearn.log_model(
            best_model,
            "model",
            registered_model_name="RidgeRegression_StudentScores_Optuna",
            input_example=X_train.iloc[:5],
            signature=mlflow.models.signature.infer_signature(X_train, y_train),
        )

        # Generate submission file
        generate_submission(best_model, "ridge_regression_submission.csv")
        mlflow.log_artifact("ridge_regression_submission.csv")
...
def main(n_trials=100):
    ...
    print("=" * 60)
    print("STUDENT TEST SCORES PREDICTION - RIDGE REGRESSION (OPTUNA)")
    print("=" * 60)
